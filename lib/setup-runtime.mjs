...
import { HOSTING_PROVIDERS, hostingStatus, normalizeEndpoint, normalizeHostingRequest } from "./hosting.mjs";
...
function hostedGatewayServer() {
  const registered = loadRegistry().servers?.naavos_gateway;
  if (registered) return registered;
  const endpoint = loadSetupState().hosting?.endpoint;
  return endpoint ? { id: "naavos_gateway", label: "Online Avatar endpoint", endpoint, auth: "oauth" } : null;
}

function hostedGatewayUrl() {
  return process.env.RADOS_NAAS_GATEWAY_URL ?? loadSetupState().hosting?.endpoint ?? hostedGatewayServer()?.endpoint ?? null;
}
...
function authorityStatus(state = null) {
  ...
  return {
    ...
    hosted_gateway: {
      ...
      optional_for: ["local_avatar"],
      needed_for: ["online_chatgpt", "online_claude"],
      release_gate: "individual_local | individual_hosted | public"
    }
  };
}
...
export async function inspectHostedGateway({ fetchImpl = fetch } = {}) {
  ...
  if (!configured) return { status: "not_configured", optional_for: ["local_avatar"], needed_for: ["online_chatgpt", "online_claude"] };
  let parsed;
  try {
...
export async function setupStatus() {
  ...
  return {
    product: "NAAvOS Avatar OS",
    authority: authorityStatus(state),
    release: {
      ...release,
      label: release.public_release
        ? "Public hosted release"
        : release.release_verified && release.channel === "individual_hosted"
          ? "Verified individual hosted handoff"
...
export function getHostingStatus() {
  return hostingStatus(loadSetupState());
}

export function configureHosting(request = {}) {
  const state = loadSetupState();
  const backup = snapshot("hosting-profile");
  try {
    const hosting = normalizeHostingRequest(request);
    state.hosting = hosting;
    state.setup.last_backup_id = backup.id;
    recordAudit(state, "hosting.configured", {
      backup_id: backup.id,
      mode: hosting.mode,
    ...
    saveSetupState(state);
    return { hosting: getHostingStatus(), backup };
  } catch (error) {
    recordSetupFailure(backup, error, state);
    throw error;
  }
...
export function openHostingProvider({ provider, instanceUrl } = {}) {
  const selected = HOSTING_PROVIDERS[provider];
  if (!selected) throw new Error("Choose a supported hosting provider");
  const url = provider === "coolify" ? normalizeEndpoint(instanceUrl) : selected.console_url;
  if (!url) throw new Error("Enter your Coolify instance URL before opening it");
  return { ...openExternal(url), provider, ownership: selected.ownership };
}
  ...
  const server = http.createServer(async (request, response) => {
    ...
    try {
      ...
      if (request.method === "GET" && url.pathname.startsWith("/api/")) {
        ...
        if (url.pathname === "/api/hosting/status") return sendJson(response, 200, getHostingStatus());
      ...
      if (request.method === "POST") {
        ...
        if (url.pathname === "/api/hosting/configure") return sendJson(response, 200, configureHosting(body));
        if (url.pathname === "/api/hosting/open") return sendJson(response, 200, openHostingProvider(body));
