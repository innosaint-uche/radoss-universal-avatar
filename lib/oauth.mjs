...
const TOKEN_SERVICE = process.env.RADOS_HOME ? "naavos-isolated-test" : "radoss-universal-avatar";
...
class SecretToolTokenStore {
  ...
  set(key, value) {
    execFileSync("secret-tool", ["store", "--label", "NAAvOS Avatar OS OAuth", "service", this.service, "provider", key], {
  ...
  try {
    ...
    callbackServer = http.createServer(async (request, response) => {
      ...
      try {
        ...
        response.end(callbackPage("Connected", "Your account is connected to NAAvOS Avatar OS."));
        resolve({
          provider: server.id,
          mode: "pkce_loopback",
