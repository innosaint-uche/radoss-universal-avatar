...
const app = path.join(bundleDir, "macos", "NAAvOS Avatar OS.app");
...
const packagedApp = path.join(tempDir, path.basename(app));
let mounted = false;
try {
  // Never mutate the Tauri output in place. Ad-hoc signing is applied only
  // to the DMG staging copy, leaving the verified app launchable for QA.
  fs.cpSync(app, packagedApp, { recursive: true });
  run("/usr/bin/codesign", ["--force", "--deep", "--sign", "-", packagedApp]);
  run("/usr/bin/codesign", ["--verify", "--deep", "--strict", packagedApp]);
  run("/usr/bin/hdiutil", ["create", "-volname", "NAAvOS Avatar OS", "-srcfolder", packagedApp, "-ov", "-format", "UDZO", tempDmg]);
  fs.renameSync(tempDmg, dmg);
  run("/usr/bin/hdiutil", ["attach", "-nobrowse", "-readonly", "-mountpoint", mountDir, dmg]);
  mounted = true;
  const embeddedApp = path.join(mountDir, "NAAvOS Avatar OS.app");
